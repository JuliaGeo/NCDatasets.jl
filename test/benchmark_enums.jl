...
# 18.229 μs (5 allocations: 97.79 KiB)
...
# Create a parametric struct for NetCDF enums

function reconstruct_enum_type(ncid,typeid)
    ...
    names = (first.(members)..., )
    values = (last.(members)..., )
    return NCEnum{T,Symbol(typename),names,values}
end

ETb = reconstruct_enum_type(ncid,typeid)
data4b = @btime load_enum(ETb,ncid,varid,len);
#  18.272 μs (5 allocations: 97.79 KiB)


@test data3 == reinterpret(Clouds.cloud_class_t,data4b)
